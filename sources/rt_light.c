...
static t_xy			rt_get_l_intensity(t_ray ray, t_light *l, t_scene *s, t_vec norm)
{
	t_rtres	rt;
	t_vec	l_dir;
	t_xy	res;
...
uint32_t		rt_calc_light(t_ray ray, t_scene *s, t_rtres rtres)
{
	...
	while (i < s->lnum)
	{
		if (s->light[i].type == e_point)
		{
			...
			res.t.x += dif_shin.x;
			res.colr.val = colr_add(res.colr.val, colr_scal(colr_scal(s->light[i].colr.val, dif_shin.y),
					/*res.t.x*/dif_shin.x));
		}
